...
import {
  DEFAULT_PLAYER_DATA,
  DefaultPlayerData,
} from "./Types/Templates/DefaultPlayerData";
...
love.load = (): void => {
  ...
  const PLAYER_DATA: DataStorage<DefaultPlayerData> =
    datastore.getDataStorage<DefaultPlayerData>(
      DATASTORE_NAMES.PLAYER_DATA,
      DEFAULT_PLAYER_DATA,
    );

  const SETTING_DATA: DataStorage<SettingConfig> =
    datastore.getDataStorage<SettingConfig>(
